...
void fit_na61_pbpb30() {
  ...
  TF1 eneg_delta("eneg_delta", "gaus", pMin, pMax);
  ...
  eneg.SetParVariation({{1,1e6}, {0,0}, {0.5,0.5}, {1,1}});
  ...
  TF1 pionneg_delta("pionneg_delta", "gaus", pMin, pMax);
  ...
  pionneg.SetParLimits({{1,1.6e7}, {0,7}, {0,1}, {-.1,.2}});
  pionneg.SetParVariation({{1,1.6e7}, {0.02,0.5}, {0.5,0.1}, {1,0.5}});
...
//  TF1 kaonneg_sigma("kaonneg_sigma", "pol1", pMin, pMax);
  ...
  kaonneg.SetParVariation({{1,1e6}, {0.01,0.01}, {.1,.1}, {2,1}});
  kaonneg.SetParFitLimits({{4, 6.2}, {pMin, pMax}, {4, 6.2}, {pMin, pMax}});
  ...
  pionneg.SetParVariation({{0.5,0.25}, {0.01,0.01}, {0.01,0.01}, {0.01,0.50}});
  eneg.SetParVariation   ({{0.5,0.25}, {0.01,0.01}, {0.01,0.01}, {0.01,0.50}});
...
//  bgneg1.SetOutputParametrizationFunction(0, TF1("bgneg1_amp", "", pMin, pMax));
  ...
  pionpos.SetParLimits({{1,2e7}, {0,7}, {0,.5}, {-.1,.2}});
  pionpos.SetParVariation({{1,2e7}, {0.01,0.05}, {0.05,0.05}, {1,0.5}});
  pionpos.SetParFitLimits({{pMin, pMax}, {pMin, pMax}, {0.5, 5}, {pMin, pMax}});
  ...
  TFile outfile("pbpb30.pid.root", "recreate");
